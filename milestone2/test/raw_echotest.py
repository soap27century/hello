    ...
    if max_len < 60:
        ...
        if error_bytes:
            ...
            while error_bytes:
                ...
                offset_string = "({} bytes)...".format(next_offset -
                                                       last_offset).encode()
                line3 += offset_string + b"^"
                line1 += b"." * len(
                    offset_string) + data1[next_offset:next_offset + 1]
                line2 += b"." * len(
                    offset_string) + data2[next_offset:next_offset + 1]
                last_offset = next_offset
            s = b"\n".join([line1, line2, line3])
        else:
            s = "\t{}...({} bytes)...{}\n".format(data1[:5], len(data1),
                                                  data1[-5:]).encode()
            s += "\t{}...({} bytes)...{}\n".format(data2[:5], len(data2),
                                                   data2[-5:]).encode()
            s += b"\n"
    return s
...
if __name__ == "__main__":
    ...
    if args.mode == "server":
        if args.msg != None or args.count != None:
            ...
            sys.exit(-1)
        # print(args.stack)
        print("echotest creating server")
        ...
        print("raw echo test connecting to {}://{}:{}".format(
            args.stack, args.mode, args.port))
        print("echotest client")
        coro = playground.create_connection(
            lambda: RawEchoClient(data_to_send),
            host=args.mode,
            port=args.port,
            family=args.stack)
        transport, protocol = asyncio.get_event_loop().run_until_complete(coro)
        print("Connected. Transmitted {} bytes. Wait for response".format(
            len(protocol.sent_data)))
        asyncio.get_event_loop().run_forever()
        if protocol.received_data == None:
            print(
                "Echo client transmission failed. Data was never received from the server."
            )
        elif protocol.sent_data == protocol.received_data:
            print(
                "Echo client round-trip transmission completed with no errors:"
            )
            print(
                create_compare_data(protocol.sent_data,
                                    protocol.received_data).decode())
        else:
            print(
                "Echo client round-trip transmission completed but there were errors."
            )

            correct_bytes = 0
            for i in range(len(protocol.received_data)):
                if i < len(protocol.sent_data) and protocol.sent_data[
                        i] == protocol.received_data[i]:
                    correct_bytes += 1

            print("\tData Transmitted: {} bytes".format(len(
                protocol.sent_data)))
            ...
            print(
                create_compare_data(
                    protocol.sent_data,
                    protocol.received_data).decode(errors="replace"))
